...
protected:
    ...
    void addAssay( assay_index_t assayIx ) {
        if ( !_assayIndices.empty() && _assayIndices.back() != (assayIx-1) ) {
            THROW_EXCEPTION( std::invalid_argument, "Assay #" << assayIx
                             << " does not immediately follow " << _assayIndices.back() );
        }
...
BOOST_CLASS_IMPLEMENTATION( cemm::bimap::OPAProbe, object_serializable )
BOOST_CLASS_TRACKING( cemm::bimap::OPAProbe, track_never )

BOOST_CLASS_IMPLEMENTATION( cemm::bimap::OPAData::MassSpectraData, object_serializable )
BOOST_CLASS_IMPLEMENTATION( cemm::bimap::OPAData, object_serializable )
